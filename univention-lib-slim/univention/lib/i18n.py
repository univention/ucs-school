...
class Locale(object):
    ...
    REGEX = re.compile(
        "(?P<language>([a-z]{2}|C|POSIX))(_(?P<territory>[A-Z]{2}))?(.(?P<codeset>[a-zA-Z-0-9]+)(@(?P<modifier>.+))?)?"  # noqa: E501
    )
    ...
    def parse(self, locale):
        ...
        if not regex:
            raise I18N_Error(
                "attribute does not match locale specification language[_territory][.codeset][@modifier]"
            )
    ...
    def set_language(self, language=None):
        ...
        try:
            self._translation = gettext.translation(
                self._domain, languages=(Translation.locale.language,), localedir=self._localedir
            )
        except IOError:
            try:
                self._translation = gettext.translation(
                    self._domain,
                    languages=("%s_%s" % (Translation.locale.language, Translation.locale.territory),),
