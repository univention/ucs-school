...
class SchoolClassModel(BaseModel):
	...
	def from_lib_model(cls, obj: SchoolClass, request: Request) -> "SchoolClassModel":
		...
		kwargs["url"] = request.url_for("get", class_name=kwargs["name"], school=obj.school)
		kwargs["users"] = [request.url_for("get", username=name_from_dn(dn)) for dn in obj.users]
		return cls(**kwargs)
	...
	def as_lib_model(self, request: Request) -> SchoolClass:
		...
		kwargs["users"] = [url_to_dn(request, "user", user) for user in (self.users or [])]  # this is expensive :/
...
class SchoolClassPatchDocument(BaseModel):
	...
	def to_modify_kwargs(self, school, request: Request) -> Dict[str, Any]:
		...
		if self.users:
			res["users"] = [url_to_dn(request, "user", user) for user in (self.users or [])]  # this is expensive :/
		return res
...
async def create(school_class: SchoolClassModel, request: Request) -> SchoolClassModel:
	...
	if sc.exists(get_lo_udm()):
		raise HTTPException(status_code=HTTP_409_CONFLICT, detail="School class exists.")
	else:
...
async def complete_update(
	...
	if school != url_to_name(request, "school", school_class.school):
		raise HTTPException(status_code=HTTP_422_UNPROCESSABLE_ENTITY, detail="Moving of class to other school is not allowed.")
	changed = False
