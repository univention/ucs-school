...
"""
Widget sizes. UDM uses a two-column layout and by default any widget uses one column.
Widgets can also be configured to span (partly) both columns.
"""
...
class simple(ISyntax):
    ...
    def checkLdap(self, lo, value):
        # type: ("univention.admin.uldap.access", Any) -> Any  # noqa: F821
        """
...
class gid(simple):
    ...
    error_message = _(
        "A group name must start and end with a letter, number or underscore. In between "
        "additionally spaces, dashes and dots are allowed."
    )
...
class string_numbers_letters_dots_spaces(simple):
    ...
    error_message = _(
        "Value must not contain anything other than digits, letters, dots or spaces, must be at "
        "least 2 characters long, and start and end with a digit or letter!"
    )
...
class emailAddressValidDomain(emailAddress):
    ...
    errMsgDomain = _(
        "The domain part of the following mail addresses is not in list of configured mail domains: %s"
    )
...
class primaryEmailAddressValidDomain(emailAddressValidDomain):
    ...
    errMsgDomain = _(
        "The domain part of the primary mail address is not in list of configured mail domains: %s"
    )
...
class uid_umlauts(simple):
    ...
    min_length = 1
    max_length = 16
    _re = re.compile("(?u)(^\w[\w -.]*\w$)|\w*$")  # noqa: W605
...
class UDM_Objects(ISyntax):
    ...
    """
    With `True`, only a single object can be selected using a ComboBox. With `False` multiple
    entries can be selected using a MultiObjectSelect widget.
    """
    use_objects = True
    """
    By default with `True` create Python UDM instance for each LDAP entry. With `False` only
    work with the LDAP attribute data.
    """
...
class MAC_Address(simple):
    ...
    error_message = _(
        "This is not a valid MAC address (valid examples are 86:f5:d1:f5:6b:3e, 86-f5-d1-f5-6b-3e, "
        "86f5d1f56b3e, 86f5.d1f5.6b3e)"
    )
...
class reverseLookupSubnet(simple):
    ...
    #               <-                      0-255                     ->  *dot  <-                      0-255                     ->  # noqa: E501
    regex_IPv4 = r"((([1-9]?[0-9])|(1[0-9]{0,2})|(2([0-4][0-9]|5[0-5])))\.){1,2}(([1-9]?[0-9])|(1[0-9]{0,2})|(2([0-4][0-9]|5[0-5])))"  # noqa: E501
    # normal IPv6 address without "::" substitution, leading zeroes must be preserved,
    # at most 31 nibbles
    ...
    error_message = _(
        "A subnet for reverse lookup consists of the first 1-3 octets of an IPv4 address (example: "
        '"192.168.0") or of the first 1 to 31 nibbles of an expanded (with leading zeroes and '
        'without ::-substitution) IPv6 address (example: "2001:0db8:010" for "2001:db8:100::/24")'
    )
