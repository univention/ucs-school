...
class property:
    ...
    def __init__(
        ...
        :param short_description: a short descriptive text - shown below the input filed in |UMC| by
            default.
        ...
        :param may_change: `True` if the property can be changed after the object has been created,
            `False` when the property can only be specified when the object is created.
        :param identifies: `True` if the property is part of the set of properties, which are required
            to uniquely identify the object. The properties are used by default to build |RDN| for a
            new object.
        ...
        :param prevent_umc_default_popup: `True` to prevent a pop-up dialog in |UMC| when the default
            value is not set.
        :param dontsearch: `True` to prevent searches using the property.
        :param show_in_lists: UNUSED?
        :param editable: `False` prevents the property from being modified by the user; it still can be
            modified by code.
        ...
        :param nonempty_is_default: `True` selects the first non-empty value as the default. `False`
            always selects the first default value, even if it is empty.
        :param readonly_when_synced: `True` only shows the value as read-only when synchronized from
            some upstream database.
        :param size: The |UMC| widget size; one of :py:data:`univention.admin.syntax.SIZES`.
        :param copyable: With `True` the property is copied when the object is cloned; with `False`
            the new object will use the default value.
    ...
    def default(self, object):
        ...
        # we can not import univention.admin.syntax here (recursive import) so we need to find another
        # way to identify a complex syntax
        if (
            getattr(self.syntax, "subsyntaxes", None) is not None
            and isinstance(bd0, (list, tuple))
            and not self.multivalue
        ):
            return bd0

        if isinstance(bd0, basestring):
            # multivalue defaults will only be a part of templates, so not multivalue is the common way
            # for modules
            if not self.multivalue:  # default=(template-str, [list-of-required-properties])
                ...
                # return the first element, this is only related to empty extended attributes which are
                # loaded wrong, needs to be fixed elsewhere
                if bd0:
                    return [bd0]
                return self.new()
