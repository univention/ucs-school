...
def getAdminConnection(start_tls=2, decode_ignorelist=[], reconnect=True):
    ...
    :param int start_tls: Negotiate TLS with server. If `2` is given, the command will require the
...
def getMachineConnection(  # nosec
    start_tls=2,
    decode_ignorelist=[],
    ldap_master=True,
    ...
    :param decode_ignorelist: List of LDAP attribute names which shall be handled as binary attributes.
    :type decode_ignorelist: list[str]
    :param bool ldap_master: Open a connection to the Master if True, to the preferred LDAP server
        otherwise.
...
class access(object):
    ...
    :param follow_referral bool: Follow referrals and return result from other servers instead of
        returning the referral itself.
    ...
    def __open(self, ca_certfile):
        ...
        if self.reconnect:
            ...
            self.lo = ldap.ldapobject.ReconnectLDAPObject(
                self.uri, trace_stack_limit=None, retry_max=self.client_connection_attempt, retry_delay=1
            )
        else:
        ...
        if self.protocol.lower() != "ldaps":
            if self.start_tls == 1:
                try:
                    self.__starttls()
                except Exception:  # noqa: E722  # TODO: use specific exceptions
    ...
    def __recode_entry(self, entry):
        if isinstance(entry, tuple) and len(entry) == 3:
            ...
            return dict(
                list(map(lambda k_v: (k_v[0], self.__recode_attribute(k_v[0], k_v[1])), entry.items()))
            )
        else:
            return entry
    ...
    def getAttr(self, dn, attr, required=False):
        ...
        .. warning:: the attribute name is currently case sensitive and must be given as in the LDAP
            schema

        .. warning:: when `required=True` it raises `ldap.NO_SUCH_OBJECT` even if the object exists but
    ...
    def search(
        ...
        # type: (str, str, str, List[str], bool, bool, int, int, Optional[List[ldap.controls.LDAPControl]], Optional[Dict]) -> List[Tuple[str, Dict[str, List[str]]]]  # noqa: E501
        """
        Perform LDAP search and return values.

        :param str filter: LDAP search filter.
        :param str base: the starting point for the search.
        :param str scope: Specify the scope of the search to be one of `base`, `base+one`, `one`, `sub`,
            or `domain` to specify a base object, base plus one-level, one-level, subtree, or children
            search.
        :param attr: The list of attributes to fetch.
        :type attr: list[str]
        ...
        :param serverctrls: a list of :py:class:`ldap.controls.LDAPControl` instances sent to the server
            along with the LDAP request.
        ...
        :returns: A list of 2-tuples (dn, values) for each LDAP object, where values is a dictionary
            mapping attribute names to a list of values.
        :rtype: list[tuple[str, dict[str, list[str]]]]
        :raises ldap.NO_SUCH_OBJECT: Indicates the target object cannot be found.
        :raises ldap.INAPPROPRIATE_MATCHING: Indicates that the matching rule specified in the search
            filter does not match a rule defined for the attribute's syntax.
        """
        logger.info(
            "uldap.search filter=%s base=%s scope=%s attr=%s unique=%d required=%d timeout=%d "
            "sizelimit=%d" % (filter, base, scope, attr, unique, required, timeout, sizelimit)
        )
    ...
    def searchDn(
        ...
        # type: (str, str, str, bool, bool, int, int, Optional[List[ldap.controls.LDAPControl]]) -> List[str]  # noqa: E501
        """
        ...
        return [
            ...
            for x in self.search(
                filter, base, scope, ["dn"], unique, required, timeout, sizelimit, serverctrls, response
            )
        ]
    ...
    def _merge_policy(self, policy_dn, obj_dn, object_classes, result):
        ...
        if pattrs.get("ldapFilter"):
            try:
                self.search(
                    pattrs["ldapFilter"][0], base=obj_dn, scope="base", unique=True, required=True
                )
            except ldap.NO_SUCH_OBJECT:
                return
    ...
    def get_schema(self):
        ...
        if not self.__schema:
            self.__schema = ldap.schema.SubSchema(
                self.lo.read_subschemasubentry_s(self.lo.search_subschemasubentry_s()), 0
            )
    ...
    def add(self, dn, al, serverctrls=None, response=None):
        ...
        """
        Add LDAP entry at distinguished name and attributes in add_list=(attribute-name, old-values.
        new-values) or (attribute-name, new-values).

        :param str dn: The distinguished name of the object to add.
        :param al: The add-list of 2-tuples (attribute-name, new-values).
        :param serverctrls: a list of ldap.controls.LDAPControl instances sent to the server along with
            the LDAP request
        :type serverctrls: list[ldap.controls.LDAPControl]
        :param dict response: An optional dictionary to receive the server controls of the result.
        """
    ...
    def modify(self, dn, changes, serverctrls=None, response=None):
        # type: (str, List[Tuple[str, Any, Any]], Optional[List[ldap.controls.LDAPControl]], Optional[dict]) -> str  # noqa: E501
        """
        ...
        for key, oldvalue, newvalue in changes:
            if oldvalue and newvalue:
                ...
                val = newvalue
                if (
                    key == "krb5ValidEnd" or key == "krb5PasswordEnd"
                ) and newvalue == "0":  # TODO: move into the specific handlers
                    val = 0
    ...
    def __get_new_dn(self, dn, ml):
        ...
        new_vals = dict(
            (key.lower(), val if isinstance(val, basestring) else val[0])
            for op, key, val in ml
            if val and op not in (ldap.MOD_DELETE,)
        )
        new_rdn = ldap.dn.dn2str(
            [
                [
                    (x, new_vals.get(x.lower(), dn_vals[x.lower()]), ldap.AVA_STRING)
                    for x in [y[0] for y in rdn]
                ]
            ]
        )
    ...
    def modify_ext_s(self, dn, ml, serverctrls=None, response=None):
        # type: (str, List[Tuple[str, Any, Any]], Optional[List[ldap.controls.LDAPControl]], Optional[dict]) -> None  # noqa: E501
        """
    ...
    def rename_ext_s(self, dn, newrdn, newsuperior=None, serverctrls=None, response=None):
        # type: (str, str, Optional[str], Optional[List[ldap.controls.LDAPControl]], Optional[dict]) -> None  # noqa: E501
        """
        ...
        try:
            rtype, rdata, rmsgid, resp_ctrls = self.lo.rename_s(
                dn, newrdn, newsuperior, serverctrls=serverctrls
            )
        except ldap.REFERRAL as exc:
            if not self.follow_referral:
                raise
            lo_ref = self._handle_referral(exc)
            rtype, rdata, rmsgid, resp_ctrls = lo_ref.rename_s(
                dn, newrdn, newsuperior, serverctrls=serverctrls
            )
    ...
    def explodeDn(self, dn, notypes=False):
        ...
        :param str dn: The distinguished name.
        :param bool notypes: Return only the component's attribute values if True. Also the attribute
            types if False.
        :return: A list of relative distinguished names.
        :rtype: list[str]
        """
    ...
    def compare_dn(cls, a, b):
        ...
        True
        """
        return [sorted((x.lower(), y, z) for x, y, z in rdn) for rdn in ldap.dn.str2dn(a)] == [
            sorted((x.lower(), y, z) for x, y, z in rdn) for rdn in ldap.dn.str2dn(b)
        ]
