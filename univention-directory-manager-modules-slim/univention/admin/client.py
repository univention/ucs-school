...
class Session(object):
    ...
    def make_request(self, method, uri, data=None, **headers):
        ...
        if method in ("GET", "HEAD"):
            ...
            params = None
            json = data
        return self.get_method(method)(
            uri, params=params, json=json, headers=dict(self.default_headers, **headers)
        )
...
class Module(Client):
    ...
    def search(
        self, filter=None, position=None, scope="sub", hidden=False, superordinate=None, opened=False
    ):
        ...
        for entry in entries:
            ...
            if opened:
                yield Object(
                    ...
                    entry.get("superordinate"),
                    entry["uri"],
                )  # NOTE: this is missing last-modified, therefore no conditional request is done on
                # modification!
            else:
                yield ShallowObject(self, entry["dn"], entry["uri"])

    def create(self, properties, options, policies, position, superordinate=None):
        # type: (Dict[str, Any], Dict[str, str], List[str], "univention.admin.uldap.position", Optional[str]) -> Object  # noqa: E501, F821
        # logger.debug("***** properties=%r options=%r policies=%r position=%r superordinate=%r", properties, options, policies, position, superordinate)  # noqa: E501
    ...
    def create_template(self, position=None, superordinate=None, options=None):
        # type: (Optional["univention.admin.uldap.position"], Optional[str], Optional[Dict[str, str]]) -> Object  # noqa: E501, F821
    ...
    def object(self, co, lo, position, dn="", superordinate=None, attributes=None, options=None):
        # logger.debug("***** dn=%r position=%r superordinate=%r options=%r", dn, position, superordinate,options)  # noqa: E501
...
class Object(Client):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # type: (Module, Optional[str], Dict[str, Any], Dict[str, bool], Dict[str, Any], str, str, str, Optional[Any], Optional[Any], *Any, **Any) -> None  # noqa: E501
