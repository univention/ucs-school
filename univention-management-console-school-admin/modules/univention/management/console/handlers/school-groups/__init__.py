...
class handler( umch.simpleHandler, _revamp.Web  ):
	...
	def _delete_dn(dn, module):
		position.setDn(dn[string.find(dn,",")+1:])
		if not self.adminlo:
			self.adminlo = self._make_ldap_connection( self.configRegistry['ldap/master'] )

		object=univention.admin.objects.get(module, self.co, self.adminlo, position=position, dn=dn)
		object.open()
		try:
			object.remove()
			ud.debug( ud.ADMIN, ud.INFO, 'SCHOOLGROUPS: %d deletec' % dn )
			return None
		except:
			print "unexpectet error, delete this user manually (maybe not found):",dn
			return  "unexpectet error, delete this user manually (maybe not found):"+dn+"\n"
