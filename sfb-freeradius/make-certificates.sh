...
if test -e "$SSL_BASE/password"; then
	PASSWD=`cat "$SSL_BASE/password"`
else
	PASSWD=""
fi
...
init () {
	...
	if ! test -e "$SSL_BASE/password"; then
		touch $SSL_BASE/password
		chmod 600 $SSL_BASE/password
		makepasswd > $SSL_BASE/password
	fi
	PASSWD=`cat $SSL_BASE/password`
	...
	mk_config openssl.cnf $PASSWD 10000 "Freeradius Root CA"


	openssl genrsa -des3 -passout pass:"$PASSWD" -out ${CA}/private/CAkey.pem 2048
	yes '' | openssl req -config openssl.cnf -new -x509 -days 3650 -key ${CA}/private/CAkey.pem -out ${CA}/CAcert.pem
