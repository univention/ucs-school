...
if [ -d /etc/freeradius/ssl ] ; then
	echo "Creation of new SSL-CA for freeradius has been skipped - already present"
	exit 0
fi

source /usr/lib/sfb-freeradius/make-certificates.sh
...
name="sfb-freeradius"
gencert "/etc/freeradius/ssl/$name" "$name"
chmod -R g+rx "/etc/freeradius/ssl/$name"
