        ...
        try:
            ...
            if homedir and not os.path.exists(homedir):
                if PAM_HOMEDIR_SESSION:
                    MODULE.warn("recreate homedir %r uidNumber=%r gidNumber=%r (PAM)" % (homedir, owner, group))
                    try:
                        p = PAM.pam()
                        p.start('session')
                ...
                if not os.path.exists(homedir):
                    MODULE.warn("recreate homedir %r uidNumber=%r gidNumber=%r (makedirs)" % (homedir, owner, group))
