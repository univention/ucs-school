...
def visible_samba4_school_dcs(excludeDN=None, ldap_machine_read=None, ldap_position=None):
    ...
    res = ldap_machine_read.search(
        base=ldap_position.getDn(), filter=filter, attr=["cn", "associatedDomain"]
    )
    ...
    for (relativeDomainName, prio_weight_port) in STD_S4_SRV_RECORDS.items():
        ...
        for v in old_ucr_locations.split(" "):
            try:
                ...
                ud.debug(
                    ud.LISTENER, ud.ERROR, "%s: Error parsing UCR variable %s: %s" % (name, key, ex)
                )
                priority = None
                weight = None
                port = None
...
def trigger_sync_ucs_to_s4(ldap_machine_read=None, ldap_position=None):
    ...
    for relativeDomainName in list(_relativeDomainName_trigger_set):
        ...
        ldap_filter = "(&(univentionObjectType=dns/%s)(zoneName=%s)(relativeDomainName=%s))" % (
            _record_type,
            _local_domainname,
            relativeDomainName,
        )
            ...
            try:
                ...
                if p.returncode != 0:
                    ud.debug(
                        ud.LISTENER,
                        ud.ERROR,
                        "%s: S4 Connector restart returned %s." % (name, p.returncode),
                    )
                _s4_connector_restart = False
            finally:
                listener.unsetuid()
