    ...
    if configRegistry.is_true(var, False):
        print(
            ...
            % (
                json.dumps(
                    "PROXY %s:%s"
                    % (configRegistry.get("squid/parent/host"), configRegistry.get("squid/parent/port"))
                ),
            )
        )
    else:
...
if configRegistry.is_true("proxy/pac/exclude/networks/enabled", False):
    print(
        '        // If the requested host is in the given network, send "DIRECT" (no proxy is used) or use parent proxy:'
    )
...
if configRegistry.is_true("proxy/pac/exclude/expressions/enabled", False):
    print(
        '        // If the requested shell expression matches, send "DIRECT" (no proxy is used) or use parent proxy:'
    )
    ...
    % (
        json.dumps(
            ...
            % (
                ...
                configRegistry.get("squid/httpport", "3128"),
            )
        ),
    )
)
