...
\begin{itemize}
\item In single server environments the script \ucsCommand{create\_ou} now always uses the domaincontroller
  master as responsible domaincontroller/fileserver for the new OU. This fixes wrongly assigned fileservers
  for Marktplatz shares. Existing Marktplatz shares have to be fixed manually (\ucsBug{32095}).
\end{itemize}
...
\begin{itemize}
\item Fixed the possibility to move any file on the school server through an arbitrary file upload (\ucsBug{32176}).
\end{itemize}
