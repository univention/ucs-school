...
\item UCS@school now also supports UCC systems (non"=thin"=clients). Installation and further information
  (e.g. supported features) can be found in the UCS@school administrator manual (\ucsBug{30829}).
\end{itemize}

\section{Univention Corporate Client integration}
\begin{itemize}
\item A new UCR policy will be set by the new package \ucsName{ucs-school-ucc-integration} on UCS@school
  servers. The policy sets the \ucsUCRV{ucc/proxy/http} and \ucsUCRVSA{ucc/cups/server} which will be
...
\item To allow automatic mounting of the user's home share on UCC systems, the UCR policy mentioned above will also set the
  \ucsUCRV{ucc/mount/cifshome/server} (\ucsBug{31906}).
\end{itemize}
...
\item Domaincontroller master (single server environment) and domaincontroller slave (multi server
  ...
  \ucsUCRV{ucsschool/import/generate/policy/dhcp/dns/set\_per\_ou} to \emph{false} on domaincontroller master
  and domaincontroller slave systems before the installation of UCS@school or before updating UCS@school
  (\ucsBug{31930}).
\end{itemize}
...
\item During update and installation of \ucsUAS{} the package \ucsName{univention-squid-kerberos} will be
  ...
  \ucsUCRV{squid/krb5auth=yes} and restarting the service \ucsName{squid3} (\ucsBug{31995}).
\end{itemize}
