...
class SqliteQueue(object):
	...
	def add(self, userdn, username=None, db_commit=True):  # type: (str, Optional[bool]) -> None
		...
		if username is not None:
			...
			self.cursor.execute('insert or replace into user_queue (id, userdn, username) VALUES ((select id from user_queue where userdn = ?), ?, (select username from user_queue where userdn = ?))', (userdn, userdn, username))
		if db_commit:
			self.db.commit()
