...
class Web( object ):
	...
	def _web_distribution_project_search( self, umcobject, res ):
		...
		for project in cmddata.get('projectlist', []):
			...
			row = [ umcd.Button( project['name'], 'distribution/project', umcd.Action( req ) ), description, project['sender']['obj']['username'], len(project['recipients']), len(project['files']) ]
...
#								   search_properties = [ 'firstname', 'lastname', 'username' ]
	...
	def _web_distribution_project_show( self, umcobject, res ):
		...
		if cmddata['project']['deadline']:
			deadline = time.strftime( _('%m/%d/%Y at %H:%M'), cmddata['project']['deadline'])
		else:
			deadline = _('no deadline given - please collect data manually')
		lst.add_row( [ _('Deadline:'), deadline ] )
		lst.add_row( [ _('Sender:'), [ umcd.Image('distribution/user'), cmddata['project']['sender']['obj']['username'] ] ] )
