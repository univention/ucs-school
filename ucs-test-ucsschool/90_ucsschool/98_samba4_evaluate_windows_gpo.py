...
from univention.testing.ucs_samba import get_available_s4connector_dc
import glob
...
def windows_set_gpo_security_filter(gpo_name, permission_level, target_name, target_type, replace="False", server=""):
	...
	try:
		ret_code, stdout, stderr = Win.Set_GPPermissions(gpo_name, permission_level, target_name, target_type, replace, server)
		if ret_code != 0:
...
def windows_check_registry_key(reg_key, subkey, expected_value):
	...
	try:
		...
		if ret_code != 0:
			utils.fail("Get-ItemProperty on the Windows host returned status '%s' when 0 is expected. STDOUT: %s STDERR: %s" % (ret_code, stdout, stderr))
		print "stdout:", stdout
	except univention.winexe.WinExeFailed as exc:
...
def windows_set_gpo_registry_value(gpo_name, reg_key, value_name, value, value_type, server=""):
	...
	try:
		ret_code, stdout, stderr = Win.Set_GPRegistryValue(gpo_name, reg_key, value_name, value, value_type, server)
		if ret_code != 0:
...
class GPO_Test(object):
	...
	def __enter__(self):
		...
		windows_create_gpo(gpo_name, "GPO for %s Group %s" % (self.student_name, self.security_filter_group))
		force_drs_replication()
		force_drs_replication(direction="out")
		samba_check_gpo_exists(gpo_name)

		windows_set_gpo_registry_value(gpo_name, "HKCU\Software\Policies\Microsoft\UCSTestKey", "TestUserValueOne", self.test_user_gpo_value, "String")
		force_drs_replication()
		force_drs_replication(direction="out")
		sysvol_sync()
		sysvol_check_gpo_registry_value(gpo_name, "HKCU\Software\Policies\Microsoft\UCSTestKey", "TestUserValueOne", self.test_user_gpo_value)
		...
		windows_set_gpo_security_filter(gpo_name, 'GpoRead',
		...
		windows_set_gpo_registry_value(gpo_name, "HKLM\Software\Policies\Microsoft\UCSTestKey", "TestComputerValueTwo", self.test_machine_gpo_value, "String")
		force_drs_replication()
		force_drs_replication(direction="out")
		sysvol_sync()
		sysvol_check_gpo_registry_value(gpo_name, "HKLM\Software\Policies\Microsoft\UCSTestKey", "TestComputerValueTwo", self.test_machine_gpo_value)
	...
	def check(self):
		...
		# case 2: checks with computer GPO
		windows_check_registry_key("HKLM\Software\Policies\Microsoft\UCSTestKey", "TestComputerValueTwo", self.test_machine_gpo_value)
...
def test_exam_gpo(ucr, udm, schoolenv, windows_client):
	...
	with GPO_Test(school_dn, student_name, student_pwd, windows_client['hostname'], exam_group_name) as gpo_test:
...
if __name__ == '__main__':
	...
	# setup winexe:
	Win = univention.winexe.WinExe(domainname, domain_admin, domain_admin_password, windows_admin, windows_admin_password, 445, dns_get_host_ip(windows_client['hostname']), loglevel=0)
