...
class UCSSchoolSlaveGroupMemberships(AutoMultiSchoolEnv):
	...
	def run_all_tests(self):  # type: () -> None
		...
		expected_warnings_slave.append('Host object is member in global edu group but not in OU specific slave group (or the other way around)')

		# Add slave to OUschoola-DC-Verwaltungsnetz
		grp_dn = 'cn=OU{}-DC-Verwaltungsnetz,cn=ucsschool,cn=groups,{}'.format(self.schoolA.name, self.ucr.get('ldap/base'))
		...
		expected_warnings_slave.append('Host object is member in global admin group but not in OU specific slave group (or the other way around)')
		...
		expected_warnings_slave.append('Host object is member in global edu group but not in OU specific memberserver group (or the other way around)')

		# Add slave_dn to OU{}-Member-Verwaltungsnetz
		grp_dn = 'cn=OU{}-Member-Verwaltungsnetz,cn=ucsschool,cn=groups,{}'.format(self.schoolA.name, self.ucr.get('ldap/base'))
		unique_members = self.unique_members(grp_dn)
		unique_members.append(slave_dn)
		self.lo.modify(grp_dn, [('uniqueMember', self.unique_members(grp_dn), unique_members)])
		expected_warnings_slave.append('Host object is member in edu groups AND in admin groups which is not allowed')
		...
		logger.info('Run diagnostic tool, capture and test if warnings were raised. The dns of the client computers should appear be in the warnings.')
		...
		for warning in expected_warnings_slave:
			if warning not in warnings[slave_dn]:
				raise Exception('diagnostic tool {} did not raise warning for {}!\n'.format(module_name, warning))
