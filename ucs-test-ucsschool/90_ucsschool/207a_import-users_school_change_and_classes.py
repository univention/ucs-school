...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):  # formally test_school_change()
		...
		for role in ('student', 'teacher', 'teacher_and_staff'):
			...
			person.update(record_uid=person.username, source_uid=source_uid, school_classes={self.ou_A.name: [class_A_name]})
		...
		person = PersonWithSchool(self.ou_A.name, role)
		person.update(record_uid=person.username, source_uid=source_uid, schools=[self.ou_A.name, self.ou_B.name], school_classes={self.ou_A.name: [class_A_name], self.ou_B.name: [class_B_name]})
		person.old_groups = None
		person_list.append(person)
		...
		for person in person_list:
			...
			person.update(
				school=self.ou_B.name,
				schools=[self.ou_B.name],
				school_classes={}
			)
		fn_csv = self.create_csv_file(person_list=person_list, mapping=config['csv']['mapping'])
		fn_config = self.create_config_json(config=config)
