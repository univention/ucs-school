...
from csv import reader as csvreader, DictReader, DictWriter, excel, QUOTE_ALL
import sys
import time
import json
import random
...
class FactoryConfTest(object):
	...
	def check_new_and_removed_users(self, exp_new, exp_removed):
		...
		if len(new_users) != exp_new:
			self.logger.error('Invalid number of new users (expected %d, found %d)! Found new objects: %r', exp_new, len(new_users), new_users)
		...
		if len(removed_users) != exp_removed:
			self.logger.error('Invalid number of removed users (expected %d, found %d)! Removed objects: %r', exp_removed, len(removed_users), removed_users)
	...
	def test_result_exporter(self):
		...
		dialect.quoting = QUOTE_ALL
		field_names = (
			"line", "success", "error", "action", "role", "username", "schools", "firstname", "lastname",
	...
	def test_user_importer(self):
		...
		for user in users:
			if random.choice([True, False]):
				...
				user["Geburtstag"] = "{}-{:>02}-{:>02}".format(uts.random_int(1900, 2000), uts.random_int(1, 12), uts.random_int(1, 27))
	...
	def test_json_writer(self):
		...
		if vn_in.difference(vn_out):
			utils.fail("Input and output does not match:\nvn_in=%r\nvn_out=%r\nvn_in.difference(vn_out)=%r" % (
	...
	def run_all_tests(self):
		with UCSTestConfigRegistry() as ucr, UCSTestSchool() as schoolenv, UCSTestUDM() as udm:
			...
			self.logger.info("\n\n*** Creating OUs...\n")
			(self.ou_name, self.ou_dn), (self.ou_name2, self.ou_dn2) = schoolenv.create_multiple_ous(2, name_edudc=ucr.get('hostname'))
			self.lo = schoolenv.open_ldap_connection(admin=True)
			self.test_user_creator = TestUserCreator([self.ou_name, self.ou_name2], staff=2, students=2, teachers=2, staffteachers=2, classes=2, inclasses=2, schools=2)
