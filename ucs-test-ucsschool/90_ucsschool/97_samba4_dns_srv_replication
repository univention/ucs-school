...
class TestS4DNSSRVReplication(TestSamba4):
	...
	def get_dns_srv_location_via_udm(self, record_name):
		...
		cmd = (
			...
			'--superordinate', 'zoneName=%s,cn=dns,%s' % (self.domainname, self.ldap_base),  # needs a superordinate to work
			'--filter', 'relativeDomainName=%s' % record_name
		)

		stdout, stderr = self.create_and_run_process(cmd)
		if stderr:
			utils.fail("An error occured while trying to get the DNS SRV record '%s' via UDM: '%s'. STDERR: '%s'" % (
				record_name, " ".join(cmd), stderr))
		if not stdout.strip():
			utils.fail("No output from 'udm': '%s', while the '%s' SRV record with attributes was expected" % (
	...
	def get_dns_srv_via_univention_s4search(self, record_name, dns_searchbase=None):
		...
		if not stdout.strip():
			utils.fail("No output from 'univention-s4search', while the '%s' SRV record attributes were expected" % record_name)
	...
	def add_remove_srv_record_location_via_ldbmodify(self, record_name, action):
		...
		"""
		print("\nModifying the DNS SRV '%s' record in Samba using 'ldbmodify', %s(-ing) a test value:" % (record_name, action))

		if action not in ('add', 'delete'):
			utils.fail("The given action '%s' is not supported by the 'ldbmodify', only 'add' or 'delete' are allowed." % action)
		...
		if stderr:
			utils.fail("An error occured while trying to modify the '%s' SRV record via 'ldbmodify': '%s'. STDERR:\n '%s'" % (record_name, " ".join(cmd), stderr))
		if not stdout.strip():
			utils.fail("No output from 'ldbmodify': '%s', while the '%s' SRV record modification confirmation was expected" % (" ".join(cmd), record_name))
		print stdout
	...
	def get_samba_srv_record_location(self, record_name):
		...
		if not all((priority, weight, port, host)):
			utils.fail(
				...
				"priority=%s, weight=%s, port=%s, hostname=%s" % (record_name, priority, weight, port, host))
	...
	def whereis_dns_edit(self):
		...
		if not path.exists(dns_edit_path):
			utils.fail("The 'univention-dnsedit' or/and 'univention-directory-manager-tools' cannot be found")
	...
	def add_remove_record_location_via_dns_edit(self, records, action):
		...
		if action not in ('add', 'remove'):
			utils.fail("The given '%s' action is not a supported option for use with 'univention-dnsedit'" % action)
		...
		print("\n%s(-ing) the test location values '%s' to/from each record in '%s'" % (action.upper(), test_location, records))

		dns_edit_path = ('/usr/share/univention-directory-manager-tools/' 'univention-dnsedit')

		if not path.exists(dns_edit_path):
			print("\nThe 'univention-dnsedit' cannot be found in the default location at '%s'. Trying to determine location." % dns_edit_path)
			dns_edit_path = self.whereis_dns_edit()

		cmd = (dns_edit_path, '--binddn=' + self.UCR.get('tests/domainadmin/account'), '--bindpwd=' + self.admin_password, self.domainname, action, 'srv')
		...
		for record in records:
			...
			if stderr:
				...
				if 'Does not exist' not in stderr:
					print("An error occured while executing command '%s', STDERR: '%s'" % (" ".join(cmd + record_cmd), stderr))
	...
	def check_srv_records_equal(self, dns_srv_records):
		...
		if self.server_role == 'domaincontroller_master':
			...
			for record_name in dns_srv_records:
				print("\nChecking that DNS SRV '%s' record has the same attribute values in LDAP and Samba." % record_name)
				...
				for counter, samba_attr in enumerate(samba_attrs):
					if ldap_attrs[counter::4] != samba_attr:
						utils.fail("The '%s' record attributes in LDAP and Samba are different: '%s' vs. '%s'" % (
	...
	def get_ucr_settings_for_test(self):
		...
		try:
			...
			print("\nAn error occured while trying to get UCR settings for the test: %r. Skipping test." % exc)
			self.return_code_result_skip()
