...
class TestS4DCLocatorSharesAccess(TestSamba4):
	...
	def connect_to_master_sysvol_share(self, username, password, use_kerberos=False):
		...
		if use_kerberos:
			...
			cmd = ("smbclient", "//" + ldap_master_ip + "/sysvol", "--user=" + username + "%" + password, "--timeout=120", "--workgroup=" + domain_name.upper(), "--command=ls")
		...
		if not stdout.strip():
			utils.fail("The Samba client did not produce any output to STDOUT, while DC-Master Sysvol contents were expected")
		...
		if domain_name not in stdout:
			utils.fail("The Samba client output of DC-Master Sysvol contents does not include folder with the domain name.")
	...
	def create_student(self):
		...
		print("\nCreating a student for the test with a name '%s' and a password '%s'" % (student_name, student_password))

		student_dn = self.TestSchool.create_user(self.select_school_ou(True), username=student_name, password=student_password)[1]

		wait_for_drs_replication("(sAMAccountName=%s)" % escape_filter_chars(student_name), attrs="objectSid")
