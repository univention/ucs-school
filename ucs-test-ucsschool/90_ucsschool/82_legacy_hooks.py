...
class TestLegacyHooksMeta(type):
    def __new__(mcls, cls_name, bases, attrs):
        ...
        cls = super(TestLegacyHooksMeta, mcls).__new__(
            mcls, cls_name, bases, attrs
        )  # type: Type[TestLegacyHooks]
        ...
        for model in cls.models:
            if (
                inspect.getsourcelines(model.build_hook_line) == school_base_abc_source
                or inspect.getsourcelines(model.build_hook_line)[0][-1].strip() == "return None"
            ):
...
class TestLegacyHooks(TestCase):
    ...
    def _check_test_setup(self):
        for _dir in ("{}_{}_{}.d".format(self.hook_type, self.operation_name, t) for t in self.times):
            ...
            self.assertTrue(
                os.path.isdir(hook_file_path), "Not a / not existing dir: {!r}".format(hook_file_path)
            )
    ...
    def _test_move(self):
        ...
        if self.model is School:
            ...
            patterns_and_words = ((r"^{}_move_pre.d".format(self.hook_type), (obj.name,)),)
        for pattern, words in patterns_and_words:
            self.assertTrue(
                check_lines_for_pattern_and_words(txt.split("\n"), pattern, *words),
                "Could not find expected pattern {!r} and words {!r} in result file: "
