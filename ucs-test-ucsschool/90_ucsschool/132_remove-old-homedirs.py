...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):
		for role in ('student', 'teacher', 'teacher_and_staff'):
			...
			self.log.info('*** Creating user 2 with role %r in OUs %r', role, [self.ou_A.name, self.ou_B.name])
			kwargs = dict(
				ou_name=self.ou_A.name,
			...
			try:
				...
				self.log.error("*** getpwnam() failure -> user not found -> probably replication error: %s", exc)
