...
def main():
	def write_sg_cfg(prefix=''):
		with open(fn_cfg, 'w') as fd:
			fd.write(
				'\n'.join([
					prefix,
					CONF_SQUIDGUARD % {'tempdir': tempdir}
				])
			)
	...
	def write_lists():
		with open(fn_userlist, 'w') as fd:
			for i in xrange(1000):
				for name in ('anton', 'berta', 'caesar', 'doris', 'emil', 'frieda', 'gustav', 'hanna', 'immo', 'jane'):
					fd.write('%s%d\nMYDOMAIN\\%s%d\n' % (name, i, name, i))
		...
		with open(fn_whitelist_urls, 'w') as fd:
			for i in xrange(10000):
				fd.write('http://updates.software-univention.de/%s/%s.html\n' % (random_string(), random_string()))
	...
	try:
		...
		write_lists()
		write_sg_cfg('')
		exitcode = subprocess.call(['squidGuard', '-d', '-c', fn_cfg, '-C', 'all'], stdin=open('/dev/null', 'r'), stdout=open(fn_log, 'a+'), stderr=open(fn_log, 'a+'))
		print_sg_log()
		if exitcode:
		...
		exitcode = subprocess.call(['squidGuard', '-d', '-c', fn_cfg], stdin=open('/dev/null', 'r'), stdout=open(fn_log, 'a+'), stderr=open(fn_log, 'a+'))
		print_sg_log()
		if exitcode:
		...
		truncate_sg_log()
		write_sg_cfg('dbtemp %s' % (tempdir,))
		p = subprocess.Popen(['squidGuard', '-d', '-c', fn_cfg], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=open(fn_log, 'a+'))
		for i in xrange(100000):
			...
			if i % 3 == 1:
				...
				username = '-'
			p.stdin.write('http://www.univention.de/%s/%s.html 10.20.30.40/- %s GET\n' % (random_string(), random_string(), username))
			p.stdout.readline()
		print
		# p.stdin.close()
		p.communicate()
