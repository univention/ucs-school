...
def create_pw(length, specials, num_specials_allowed):
	...
	pw_count += 1
	if len(pw) != length:
		utils.fail('Requested password of length {}, got {!r} with length {}.'.format(length, pw, len(pw)))
...
def check_pw_policy():
	...
	with udm_test.UCSTestUDM() as udm, ucr_test.UCSTestConfigRegistry() as ucr:
		for length in range(1, 21):
			...
			user_dn, username = udm.create_user(policy_reference=policy_dn, password=uts.random_string(length))
			pw = create_passwd(dn=user_dn)
			pw_count += 1
			if len(pw) != length:
				utils.fail('Generated password for DN {!r} is {!r} with length {}, but should have been {}.'.format(
...
def check_num_specials(pw, specials=None, num_specials_allowed=None):
	...
	if num_specials > num_specials_allowed:
		utils.fail('Password {!r} with length {} contains {} specials, but only {} are allowed.'.format(
...
def check_char_classes(pw, specials):
	if not any(char in (string.lowercase + string.uppercase) for char in pw):
		utils.fail('Password {!r} with length {} does not contain any lowercase or uppercase character.'.format(pw, len(pw)))

	if len(pw) >= 4:
		if not any(char in string.lowercase for char in pw):
			utils.fail('Password {!r} with length {} does not contain lowercase character.'.format(pw, len(pw)))

		if not any(char in string.uppercase for char in pw):
			utils.fail('Password {!r} with length {} does not contain uppercase character.'.format(pw, len(pw)))
		...
		if specials != '' and len(pw) / 5 > 0:
			if not any(char in specials for char in pw):
				utils.fail('Password {!r} with length {} does not specials (argument specials={!r}).'.format(
					pw, len(pw), specials))
...
def check_forbidden_chars(pw):
	for char in pw:
		if char in forbidden_chars:
			utils.fail('Password {!r} with length {} contains forbidden character {!r}.'.format(
				pw, len(pw), char)
			)
...
def main():
	...
	if len(pw) != default_length:
		utils.fail('Requested password of length {}, got {!r} with length {}.'.format(default_length, pw, len(pw)))
	check_num_specials(pw)
	check_forbidden_chars(pw)
