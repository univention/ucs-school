...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):
		...
		def test_custom_settings():
			self.log.info('*** 2/4: Importing a user from each role (custom username:max_length settings)...')
			...
			username_lengths['default'] = random.randint(9, 20)  # min length is 4, if student is unset and default is used, it must be at least 4 + len('exam-') = 9
		...
		def test_settings_higher_than_20(set_ucr, counter):
			self.log.info('*** %s: Importing a user from each role (custom username:max_length settings higher than 20) and UCR is%s set...', counter, '' if set_ucr else ' NOT')
			...
			self.log.info('*** UCRV ucsschool/username/max_length = %r', self.ucr.get("ucsschool/username/max_length"))
			...
			try:
				self.run_import(['-c', fn_config])
				if not set_ucr:
					self.fail('Import should have failed, because UCRV was not set and username:max_length:* to high.')
		...
		test_settings_higher_than_20(False, '3/4')  # without setting the UCRV, settings > 20 should be ignored
