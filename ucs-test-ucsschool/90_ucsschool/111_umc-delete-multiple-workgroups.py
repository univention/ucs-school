#!/usr/share/ucs-test/runner python
## -*- coding: utf-8 -*-
## desc: test deleting multiple workgroups
## roles: [domaincontroller_slave]
## tags: [ucsschool]
## bugs: [47393]
## exposure: dangerous
## packages: [ucs-school-umc-groups]

#
# Hint: When debugging interactively, disable output capturing:
# $ pytest -s -l -v ./......py::test_create
#
import pytest

import univention.testing.ucr as ucr_test
import univention.testing.ucsschool.ucs_test_school as utu
import univention.testing.utils as utils
from univention.testing.ucsschool.workgroup import Workgroup
from univention.testing.umc import Client


def test_delete_multiple_workgroups():
    ucr = ucr_test.UCSTestConfigRegistry()
    ucr.load()
    host = ucr.get("hostname")
    total_groups = 6
    remaining_groups = 2
    with utu.UCSTestSchool() as schoolenv:
        school, oudn = schoolenv.create_ou(name_edudc=host)
        connection = Client.get_test_connection()
        hitlist = []
        # e.g.: Create 5 workgroups and remove 4 at once. Check if exactly one group remains
        for i in range(total_groups):
            work_group = Workgroup(school, connection=connection)
            work_group.create()
            utils.wait_for_replication()
            work_group.verify_exists(group_should_exist=True, share_should_exist=True)
            if i < total_groups - remaining_groups:
                hitlist.append(work_group.dn())

        res_remove = connection.umc_command(
            "schoolgroups/remove", [{"object": hitlist}], "workgroup-admin"
        ).result
        assert res_remove[0]["success"]

        res_query = connection.umc_command(
            "schoolgroups/query", {"school": school, "pattern": ""}, "workgroup-admin"
        ).result
        assert len(res_query) == remaining_groups


# TODO: Do we have to ensure this? (Bug #47393)
"""
def test_query_groups_from_other_school():
    ucr = ucr_test.UCSTestConfigRegistry()
    ucr.load()
    host = ucr.get("hostname")
    with utu.UCSTestSchool() as schoolenv:
        school_1, oudn_1 = schoolenv.create_ou(name_edudc=host)
        school_2, oudn_2 = schoolenv.create_ou(name_edudc=host)
        admin_name, admin_dn = schoolenv.create_school_admin(
            school_1,
            is_staff=False,
            is_teacher=True
        )
        connection = Client(hostname=host, username=admin_name, password="univention")

        work_group = Workgroup(school_2, connection=connection)
        work_group.create()
        utils.wait_for_replication()

        res_query = connection.umc_command(
            "schoolgroups/query", {"school": school_2, "pattern": ""}, "workgroup-admin"
        ).result
        assert not len(res_query)
"""


if __name__ == "__main__":
    assert pytest.main(["-l", "-v", __file__]) == 0
