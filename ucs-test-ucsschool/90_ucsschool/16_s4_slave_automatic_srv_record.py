...
from ucsschool.lib.school_umc_ldap_connection import set_credentials, LDAP_Connection, MACHINE_READ
...
class Test:
    ...
    def run(self, ldap_machine_read=None, ldap_position=None):
        ...
        try:
            ldap_filter = (
                "(&(objectClass=univentionDomainController)(!(univentionService=%s)))"
        ...
        with testing_udm.UCSTestUDM() as udm:
            for school in School.get_all(ldap_machine_read):
                ...
                wait_for_drs_replication(
                    "(sAMAccountName=%s)" % (escape_filter_chars(dummy_username),),
                ...
                ucr_var = "connector/s4/mapping/dns/srv_record/%s.%s/location" % (
                    test_srv_record,
                    ucr.get("domainname"),
                )
                ...
                if stdout.find(positive_test_fqdn) == -1:
                    testing_utils.fail(
                        log_message="%s not found in DNS SRV record %s"
                        % (positive_test_fqdn, test_srv_record)
                    )

                # verify that the negative_test DC is NOT present in DNS/Samba4
                if stdout.find(negative_test_fqdn) != -1:
                    testing_utils.fail(
                        log_message="%s found in DNS SRV record %s"
                        % (negative_test_fqdn, test_srv_record)
                    )
        ...
        with testing_udm.UCSTestUDM() as udm:
            ...
            testing_utils.wait_for_replication_and_postrun()
            wait_for_drs_replication(
                "(sAMAccountName=%s)" % (escape_filter_chars(dummy_username),), attrs="sAMAccountName"
            )
        # The replication in a multi shool environment need more time
        time.sleep(15)
        ...
        p1 = subprocess.Popen(
            ["host", "-t", "srv", "_kerberos._tcp"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        (stdout, stderr) = p1.communicate()
        for postitive_test_fqdn in positive_test_fqdn_list:
            if stdout.find(postitive_test_fqdn) != -1:
                testing_utils.fail(
                    log_message="%s still found in DNS SRV record %s"
        ...
        for negative_test_fqdn in negative_test_fqdn_list:
            if stdout.find(negative_test_fqdn) != -1:
                testing_utils.fail(
                    log_message="%s present in DNS SRV record %s" % (negative_test_fqdn, test_srv_record)
                )
        ...
        if stdout.find(local_fqdn) == -1:
            testing_utils.fail(
                log_message="%s not present in DNS SRV record %s" % (local_fqdn, test_srv_record)
            )
        ...
        for positive_test_fqdn in positive_test_fqdn_list:
            if test_value.find(positive_test_fqdn) != -1:
                testing_utils.fail(
                    log_message="%s still found in UCR variable %s" % (positive_test_fqdn, ucr_var)
                )
        ...
        for negative_test_fqdn in negative_test_fqdn_list:
            if test_value.find(negative_test_fqdn) != -1:
                testing_utils.fail(
                    log_message="%s found in UCR variable %s" % (negative_test_fqdn, ucr_var)
                )
