...
from re import search, compile as regex_compile
...
class TestS4SlaveUserCreationDenied(TestSamba4):
	...
	def create_samba4_user(self, creation_options):
		...
		print("\nAttempting to create a user with a username '%s' and options: %s" % (username, creation_options))
		...
		if stdout:
			...
			if bool(search(".*User .* created successfully.*", stdout)):
				...
				utils.fail("The creation of user '%s' succeded, while should be disabled on DC-Slave with Samba4" % username)

		if not stderr.strip():
			utils.fail("Expecting the user creation to fail, however, the 'samba-tool' did not produce any output to STDERR.")

		if not bool(self.fail_pattern.match(stderr)):
			utils.fail("The 'samba-tool' output produced to STDERR does not match the '%s' pattern. STDERR: '%s'" % (
