...
class TestCases(object):
	def __init__(self, ucr, schoolenv):
		...
		self.school2 = MySchool(*self.schoolenv.create_ou(name_edudc='dcschool2', wait_for_replication=False))
		...
		self.student0 = MyStudent(*self.schoolenv.create_student(self.school1.name, wait_for_replication=False))
		self.student1 = MyStudent(*self.schoolenv.create_student(self.school1.name, wait_for_replication=False))
		self.student2 = MyStudent(*self.schoolenv.create_student(self.school2.name, schools=[self.school1.name, self.school2.name], wait_for_replication=False))
		self.teacher0 = MyTeacher(*self.schoolenv.create_teacher(self.school1.name, wait_for_replication=False))
		self.teacher1 = MyTeacher(*self.schoolenv.create_teacher(self.school1.name, wait_for_replication=False))
		self.teacher2 = MyTeacher(*self.schoolenv.create_teacher(self.school2.name, schools=[self.school1.name, self.school2.name], wait_for_replication=False))
		is_teacher = True if self.ucr.get('server/role') == 'domaincontroller_slave' else None
		self.admin0 = MyAdmin(*self.schoolenv.create_school_admin(self.school1.name, is_teacher=is_teacher, wait_for_replication=False))
		self.admin1 = MyAdmin(*self.schoolenv.create_school_admin(self.school1.name, is_teacher=is_teacher, wait_for_replication=False))
		...
		utils.verify_ldap_object(self.student2.dn, expected_attr={'ucsschoolSchool': [self.school1.name, self.school2.name]}, strict=True, should_exist=True)
		utils.verify_ldap_object(self.teacher2.dn, expected_attr={'ucsschoolSchool': [self.school1.name, self.school2.name]}, strict=True, should_exist=True)
	...
	def test_pw_reset(self, actor, target, expected_result):
		...
		for attr_name in ('sambaNTPassword', 'userPassword', 'pwhistory', ):   # "krb5key" has no eq matching rule, so lo.modify fails
