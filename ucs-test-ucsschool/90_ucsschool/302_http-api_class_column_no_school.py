...
from csv import DictReader, DictWriter, excel, QUOTE_ALL
	...
	def test(self):
		...
		with tempfile.NamedTemporaryFile() as tmpfile1:
			...
			if import_job.status == 'Finished':
				...
				self.fail(
					'Import failed: status={!r} result={!r}'.format(import_job.status, import_job.result.result),
		...
		with tempfile.NamedTemporaryFile() as tmpfile1, tempfile.NamedTemporaryFile() as tmpfile2:
			...
			if import_job.status == 'Aborted':
				self.log.info('OK: import failed: status=%r result=%r', import_job.status, import_job.result.result)
			else:
				self.fail(
					'Import succeeded: status={!r} result={!r}'.format(import_job.status, import_job.result.result),
					import_job=import_job)
