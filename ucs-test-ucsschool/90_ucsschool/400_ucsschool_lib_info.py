...
def test_ucsschool_lib_info(udm_session, schoolenv, ucr):
    ...
    school, oudn = schoolenv.create_ou(name_edudc=name_edudc, name_admindc=name_admindc, use_cache=False)
    ...
    dn_edudc = lo.searchDn(
        filter="(&(univentionObjectType=computers/domaincontroller_slave)(cn={}))".format(name_edudc),
        base=oudn,
    )[0]
    dn_admindc = lo.searchDn(
        filter="(&(univentionObjectType=computers/domaincontroller_slave)(cn={}))".format(name_admindc),
        base=oudn,
    )[0]
    ...
    assert not (uli.is_school_slave(lo, dn_centraldc)), "{} should NOT be a school slave".format(
        dn_centraldc
    )
    with pytest.raises(
        ValueError,
    ...
    assert not (uli.is_central_computer(lo, dn_edudc)), "{} should NOT be a central slave".format(
        dn_edudc
    )
    assert not (uli.is_central_computer(lo, dn_admindc)), "{} should NOT be a central slave".format(
        dn_admindc
    )
    assert uli.is_central_computer(lo, dn_centraldc), "{} should be a central slave".format(dn_centraldc)
    assert uli.is_central_computer(lo, dn_memberserver), "{} should be a central memberserver".format(
        dn_memberserver
    )
