...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):
		...
		config.update_entry('scheme:email', '<:umlauts><firstname:lower>.<lastname:lower>[ALWAYSCOUNTER]@<maildomain>'),
		...
		univention.config_registry.handler_unset(['ucsschool/import/generate/user/attributes/no-overwrite-by-schema'])
		...
		for person in person_list:
			...
			if not local.endswith('1'):
				self.fail('Email address should end in "1" ([ALWAYSCOUNTER] in scheme:email), but is {!r}.'.format(email))
		...
		univention.config_registry.handler_set(['ucsschool/import/generate/user/attributes/no-overwrite-by-schema=uid'])
		self.save_ldap_status()
		self.run_import(['-c', fn_config])
		self.check_new_and_removed_users(0, 0)
		for person in person_list:
		...
		univention.config_registry.handler_set(['ucsschool/import/generate/user/attributes/no-overwrite-by-schema=mailPrimaryAddress uid'])
		self.save_ldap_status()
		self.run_import(['-c', fn_config])
		self.check_new_and_removed_users(0, 0)
		for person in person_list:
