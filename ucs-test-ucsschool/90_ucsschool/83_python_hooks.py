...
from ucsschool.lib.models.base import UCSSchoolHelperAbstractClass, PYHOOKS_PATH, _pyhook_loader
...
from ucsschool.lib.models.utils import env_or_ucr, get_stream_handler, UniStreamHandler
...
MODULE_PATHS = (
    ("/kelvin/ucs-school-lib/modules/ucsschool/lib/models", "ucsschool.lib.models"),
    ("/kelvin/ucs-school-import/modules/ucsschool/importer/models", "ucsschool.importer.models"),
    ("/kelvin/ucs-school-import/modules/ucsschool/importer/legacy", "ucsschool.importer.legacy"),
)
...
fake = Faker()
random_ip = fake.ipv4_private
random_mac = fake.mac_address
...
def get_ucsschool_logger() -> logging.Logger:
    force_ucsschool_logger_colorized_if_has_tty()
    _logger = logging.getLogger('ucsschool')
    _logger.setLevel(logging.DEBUG)
    if not any(isinstance(handler, UniStreamHandler) for handler in _logger.handlers):
        _logger.addHandler(get_stream_handler('DEBUG'))
    return _logger
...
def get_test_ous() -> List[Tuple[str, str]]:
    res = []
    lo: LoType = getAdminConnection()[0]
    for dn, attrs in lo.search("(&(objectClass=ucsschoolOrganizationalUnit)(ou=testou*))"):
        res.append((attrs["ou"][0].decode(), dn))
    return res[:2]
...
class TestPythonHooks(IsolatedAsyncioTestCase):
    """
    ...
    def setUpClass(cls):
        ...
        (cls.ou_name, cls.ou_dn), (cls.ou2_name, cls.ou2_dn) = get_test_ous()
    ...
    async def tearDownClass(cls):
        if cls._dhcp_service:
            ...
            async with UDM(**udm_kwargs()) as udm:
                await cls._dhcp_service.remove(udm)
    ...
    async def test_002_subclassing(self):
        ...
        for klass in ("Staff", "Student", "Teacher", "TeachersAndStaff"):
            ...
            obj = await getattr(self, "_create_{}".format(klass))()
    ...
    async def _test_create(self):
        ...
        try:
            obj = await getattr(self, "_create_{}".format(self.model.__name__))()
        except AttributeError:
    ...
    async def _test_move(self):
        ...
        if hasattr(obj, "schools"):
            async with UDM(**udm_kwargs()) as udm:
                await obj.change_school(self.ou2_name, udm)
            move_success = True
        else:
    ...
    async def _create_ExamStudent(self):
        return self.model(
            school=self.ou_name,
            name=uts.random_username(),
    ...
    async def _create_School(self):
        return self.model(
            name=uts.random_username(),
        )

    async def _create_IPComputer(self):
        return self.model(
            school=self.ou_name,
            name=uts.random_username(),
    ...
    async def _create_MarketplaceShare(self):
        # there can be only one MarketplaceShare, and the test OU already has one
        async with UDM(**udm_kwargs()) as udm:
            await self.model(school=self.ou_name).remove(udm)
        return self.model(school=self.ou_name)

    async def _create_Container_(self):
    ...
    async def _create_DHCPServer(self):
        ...
        async with UDM(**udm_kwargs()) as udm:
            if not await self.__class__._dhcp_service.create(udm):
    ...
    async def _create_DHCPSubnet(self):
        return self.model(
            school=self.ou_name,
            name="11.40.232.0",
            dhcp_service=self.__class__._dhcp_service,
    ...
    async def _create_DNSReverseZone(self):
        return self.model(
            school=self.ou_name,
    ...
    async def _create_SchoolClass(self):
    ...
    async def _create_ImportStaff(self):
