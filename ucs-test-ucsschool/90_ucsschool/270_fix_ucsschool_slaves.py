...
def main():
	with utu.UCSTestSchool() as schoolenv:
		...
		attrs = result[0][1]
		if 'univentionWindows' in attrs.get('objectClass', []) or 'ucsschoolComputer' in attrs.get('objectClass', []):
		...
		for value in attrs.get('ucsschoolRole', []):
			if value.startswith('win_computer:school:'):
				print("WARNING: domaincontroller_slave's ucschoolRole already broken! {!r}".format(value))

		lo.modify(
			dcdn,
			[
				['objectClass', attrs.get('objectClass', []), list(set(attrs.get('objectClass', [])) | set(['univentionWindows', 'ucsschoolComputer']))],
				['ucsschoolRole', attrs.get('ucsschoolRole', []), list(set(attrs.get('ucsschoolRole', [])) | set(['win_computer:school:{}'.format(school)]))],
			])
		...
		broken = False
		result = lo.search(base=dcdn)
		if 'univentionWindows' in result[0][1].get('objectClass', []) or 'ucsschoolComputer' in result[0][1].get('objectClass', []):
