...
def test_valid_hostname(ucr, schoolenv):
            ...
            def process_school(school, dc_name, should_fail=True):
                try:
                    ...
                    assert not should_fail, "Creating a school(%s) with dc_name=%s was possible, expected to fail)" % (school, dc_name,)
                    print("Creating a school(%s) with dc_name=%s, expected to succeed" % (school, dc_name,))
                except ValidationError as ex:
                    assert should_fail and "dc_name" in str(ex), "Creating a school(%s) with dc_name=%s, expected to fail: %s)" % (school, dc_name, str(ex),)
            ...
            def process_school_cli(school, dc_name, should_fail=True):
                ...
                try:
                    ...
                    assert not should_fail, "Creating a school(%s) cli with dc_name=%s was unexpectedly successful" % (school, dc_name)
                except subprocess.CalledProcessError:
                    assert should_fail, "Creating a school(%s) cli with dc_name=%s failed unexpectedly" % (school, dc_name)
            ...
            def process_school_umcp(school, dc_name, should_fail=True):
                ...
                try:
                    ...
                    assert should_fail and "DC Name:" in str(ex), "Creating a school(%s) umcp with dc_name=%s, expected to fail: %s)" % ( school.name, dc_name, str(ex),)
                else:
                    assert not should_fail, "Creating a school(%s) umcp with dc_name=%s was unexpectedly successful" % (school.name, dc_name)
                finally:
                    if created:
                        school.remove()
