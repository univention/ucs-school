...
class TestSYSVOLReplicationMultiSchool(TestSamba4):
	...
	def execute_cmd_on_host(self, host, command_line):
		...
		cmd = ('univention-ssh', '-timeout', '120', '/dev/stdin', self.admin_username + '@' + host, command_line)
		...
		if stderr:
			...
			if bool(match('Warning|Warnung', stderr, IGNORECASE)):
				...
				utils.fail("An error occured while connecting to the remote host '%s' via ssh or executing the command: '%s'" % (host, stderr))
		if stdout:
			stdout = stdout.strip()
			print("\nThe output of the command '%s' executed on the remote host '%s' is: '%s'" % (command_line, host, stdout))
			return stdout
	...
	def check_sysvol_replication(self, host):
		...
		sysvol_path = ("/var/lib/samba/sysvol/%s/Policies/%s/" % (self.UCR['domainname'], self.gpo_reference))
		...
		if not remote_stdout:
			utils.fail("The command '%s' executed on the remote host '%s' produced no output to 'stdout'" % (command_line, host))
		if remote_stdout != 'True':
			...
			utils.fail(
				...
				"exists. Command's stdout from the remote host: '%s'" % (host, self.gpo_reference, remote_stdout))
	...
	def run_local_sysvol_replication(self):
		...
		if stderr:
			utils.fail("An error occured while running the replication script '%s': '%s'" % (repl_script, stderr))
	...
	def get_other_dc_hostnames(self):
		...
		cmd = ("univention-ldapsearch", "-h", ldap_master, "-p", self.UCR.get('ldap/master/port'), "-D", self.UCR.get('tests/domainadmin/account'), "-w", self.admin_password, search_pattern, search_attribute)

		search_stdout, search_stderr = self.create_and_run_process(cmd)
		if search_stderr:
			print("An error message while executing 'univention-ldapsearch' on the '%s' with pattern '%s': '%s'" % (
				ldap_master, search_pattern, search_stderr))

		# reduce the LDAP search results to only 'displayName' fields:
		stdout, stderr = self.create_and_run_process(("sed", "-n", "s/^%s: //p" % search_attribute), PIPE, search_stdout)
		if stderr:
			utils.fail(
	...
	def create_samba_gpo(self):
		...
		print("\nCreating Group Policy Object (GPO) for the test with a name '%s' using 'samba-tool'" % display_name)

		# define localhost explicitly:
		host = 'ldap://' + self.UCR.get('hostname') + '.' + self.UCR.get('domainname')

		cmd = ("samba-tool", "gpo", "create", display_name, "-H", host, "--username=" + self.admin_username, "--password=" + self.admin_password)
