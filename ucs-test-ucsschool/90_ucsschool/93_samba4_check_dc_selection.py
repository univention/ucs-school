...
class TestS4DCLocatorDNS(TestSamba4):
	...
	def check_dc_location(self):
		...
		utils.fail("None of the correct DC options were found among the located DC:\n%s" % located_dc.strip())
	...
	def populate_list_of_site_dcs(self):
		...
		if self.is_a_school_branch_site(self.UCR.get('ldap/hostdn')):
			...
			central_dcs += ' '
			central_dcs += self.sed_for_key(self.get_udm_list_dcs("domaincontroller_backup"), "^  fqdn: ")
