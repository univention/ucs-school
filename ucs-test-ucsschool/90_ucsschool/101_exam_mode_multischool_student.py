...
from ucsschool.lib.models import Student, ExamStudent
from ucsschool.lib.roles import create_ucsschool_role_string, role_exam_user, role_student, context_type_exam
	...
	with univention.testing.udm.UCSTestUDM() as udm:
		with utu.UCSTestSchool() as schoolenv:
			with ucr_test.UCSTestConfigRegistry() as ucr:
				...
				klasse_dn = udm.create_object('groups/group', name='%s-%s' % (school, uts.random_name()), position="cn=klassen,cn=schueler,cn=groups,%s" % oudn)
				klasse_dn2 = udm.create_object('groups/group', name='%s-%s' % (school2, uts.random_name()), position="cn=klassen,cn=schueler,cn=groups,%s" % oudn2)
				...
				for s in (school, school2):
					...
					# Creating the room
					schoolenv.create_computerroom(s, name=room.name, description=room.description, host_members=room.host_members)
					rooms[s] = room
				...
				exam_role_str = create_ucsschool_role_string(role_exam_user, "{}-{}".format(exam.name, school), context_type_exam)
				exam2_role_str = create_ucsschool_role_string(role_exam_user, "{}-{}".format(exam2.name, school2), context_type_exam)
				exam_student2 = ExamStudent.get_all(open_ldap_co, school2, filter_format("uid=exam-%s", (student2.name,)))[0]
				...
				exam.finish()
				exam_student2 = ExamStudent.get_all(open_ldap_co, school, filter_format("uid=exam-%s", (student2.name,)))[0]
				assert os.path.isdir(exam_student2_home)
				assert all(s in exam_student2.schools for s in (school, school2))
