...
class TestGPCReplicationTwoWays(TestSamba4):
	...
	def check_gpo_replicated_locally(self):
		...
		cmd = ("samba-tool", "gpo", "show", self.gpo_reference,
		...
		if not stdout:
			utils.fail("The 'samba-tool' did not produce any output to STDOUT, while GPO info was expected")

		gpo_dn = "CN=%s,CN=Policies,CN=System" % self.gpo_reference
		if gpo_dn not in stdout:
			utils.fail("The 'samba-tool' did not list the replicated test GPO in the 'dn', STDOUT: '%s'" % stdout)
	...
	def find_school_ou(self):
		...
		if server_role == "domaincontroller_master":
			...
			(host_dn, _) = lo.search(filter=filter_string, attr=["dn"])[0]
	...
	def create_gpo_on_remote_host(self):
		...
		print("\nCreating a Group Policy Object (GPO) on the host '%s' with a display name '%s' using 'samba-tool'" % (self.remote_host, display_name))

		cmd = ("samba-tool", "gpo", "create", display_name, self.host_or_ip, self.remote_host, "-k", "no", "--username", self.admin_username, "--password", self.admin_password)
		...
		if not stdout:
			utils.fail("The 'samba-tool' did not produce any output to STDOUT, while a GPO reference was expected")
	...
	def create_gpo_link_on_remote_host(self, container_dn):
		...
		cmd = ("samba-tool", "gpo", "setlink", container_dn, self.gpo_reference, self.host_or_ip, self.remote_host, "-k", "no", "--username", self.admin_username, "--password", self.admin_password)
		...
		if not stdout:
			utils.fail("The 'samba-tool' did not produce any output to STDOUT, while a GPO link confirmation was expected")
	...
	def check_gpo_link_replicated_locally(self, container_dn):
		...
		cmd = ("samba-tool", "gpo", "getlink", container_dn,
		                     "--ipaddress=" + self.local_ip,
		                     "--username", self.admin_username,
		                     "--password", self.admin_password)
		stdout, stderr = self.create_and_run_process(cmd)
		...
		if stderr:
			print "\nExecuting cmd:", cmd
			utils.fail("An error occured while getting the GPO link using 'samba-tool', STDERR: '%s'" % stderr)
		if not stdout:
			utils.fail("The 'samba-tool' did not produce any output to the STDOUT, while GPO list was expected")
		...
		if container_dn not in stdout:
			utils.fail(
				...
				"not replicated")
		if self.gpo_reference not in stdout:
			utils.fail("The linked GPO was not referenced in the 'samba-tool output, possibly link was not replicated")
	...
	def find_slave_in_domain(self):
		...
		if "serverRole: slave" not in udm_stdout.strip():
			...
			if not sed_stdout:
				utils.fail("Could not find at least one IP address of the DC-Slave in the output of the udm list command")

			slave_ips = sed_stdout.split()
			print("\nThe DC-Slave(s) with the following IP address(-es) were found in the domain: '%s'" % slave_ips)
	...
	def select_remote_host(self):
		...
		if server_role == 'domaincontroller_master':
			...
			print("\nThe following DC-Slave '%s' will be selected as the remote host for the test" % self.remote_host)

		elif server_role == 'domaincontroller_slave':
			# check first if DC-Master has Samba4:
			if not self.dc_master_has_samba4():
				print("The DC-Master '%s' has no Samba4, thus remote check not possible, skipping the test." % self.ldap_master)
				self.return_code_result_skip()
