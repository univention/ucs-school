...
def test(student_classes, teacher_classes, schools, ucr, remove_from_school=None, connection=None):
	...
	user = User(school=schools[0], role='student', school_classes=deepcopy(student_classes), schools=schools, connection=connection)
	user.create()
	user.verify()
	user.check_get()
	users.append(user)

	user = User(school=schools[0], role='teacher', school_classes=deepcopy(teacher_classes), schools=schools, connection=connection)
	user.create()
	user.verify()
	user.check_get()
	users.append(user)

	user = User(school=schools[0], role='staff', school_classes={}, schools=schools, connection=connection)
	user.create()
	user.verify()
	user.check_get()
	users.append(user)

	user = User(school=schools[0], role='teacher_staff', school_classes=deepcopy(teacher_classes), schools=schools, connection=connection)
	user.create()
	user.verify()
	user.check_get()
	users.append(user)
...
def main():
	with utu.UCSTestSchool() as schoolenv:
		with ucr_test.UCSTestConfigRegistry() as ucr:
			...
			users = test(student_classes, teacher_classes, [ou, ou2], ucr, ou2, connection=umc_connection)
