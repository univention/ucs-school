...
def prepare_UCR_setup(ucr):
	changes = [
		'proxy/filter/redirecttarget=http://%s.%s/blocked-by-squid.html' % (ucr.get('hostname'), ucr.get('domainname')),
...
def test_ruleset(ucr, test_settings):
	...
	for room, rule in test_settings.items():
		...
		expected_strings = [
			'\ndest blacklist-%s {\ndomainlist blacklisted-domain-%s\nurllist blacklisted-url-%s' % (quote(rule), quote(rule), quote(rule)),
			'\ndest whitelist-%s {\ndomainlist whitelisted-domain-%s\nurllist whitelisted-url-%s' % (quote(rule), quote(rule), quote(rule)),
		]
...
def main():
	with ucr_test.UCSTestConfigRegistry() as ucr:
		...
		test_ruleset(ucr, {'Raum1': 'Nur-Univention', 'Raum2': 'Nur-Univention', 'Raum-Drei': 'Nur-Univention'})
		test_ruleset(ucr, {'Raum1': 'Nur Wikipedia', 'Raum2': 'Nur Wikipedia', 'Raum-Drei': 'Nur Wikipedia'})
