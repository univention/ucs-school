...
from univention.testing.ucsschool.importusers import Person, ImportFile, UserImport
...
class Test(UniqueObjectTester):
	...
	def test(self):
		...
		self.log.info('*** 1/4 Importing a user of each role without email in input and without email in scheme...')
		...
		self.log.info('*** 2/4 Importing a user of each role with email in input and without email in scheme...')
		...
		user_import = UserImport(
			school_name=self.ou_A.name,
			nr_students=3,
			nr_teachers=3,
			nr_staff=3,
			nr_teacher_staff=3
		)
		for user in user_import.staff + user_import.students + user_import.teachers + user_import.teacher_staff:
		...
		self.log.info('*** 4/4 Doing (new) legacy import: 3 users each role, having NO email addresses...')
