...
class UCSSchoolSchoolConsistency(AutoMultiSchoolEnv):
	...
	def run_all_tests(self):  # type: () -> None
		...
		for user in (self.schoolA.teacher, self.schoolA.student):
			...
			self.lo.modify(class_dn, [
				['uniqueMember', None, user.dn],
				['memberUid', None, user.name],
			])
		...
		try:
			...
			out = module.execute(None)
			assert out and (out['success'] is False) and (out['type'] == 'warning'), 'unexpected result from test: out={!r}'.format(out)
			logger.info('Diagnostic module returned the following error message:\n%s', out['description'])
			...
			for user in (self.schoolA.teacher, self.schoolA.student):
				...
				try:
					self.lo.modify(class_dn, [
						['uniqueMember', user.dn, None],
						['memberUid', user.name, None],
					])
