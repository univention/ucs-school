...
class Test(UniqueObjectTester):
	...
	def import_test2(self):
		...
		persons = list()
		for role, schar in (('teacher', '-'), ('student', '.'), ('staff', '-'), ('teacher_and_staff', '_')):
			person = Person(self.ou_A.name, role)
			if role == 'staff':
				firstname = schar + uts.random_name(random.randint(4, 8))
				lastname = uts.random_name(4) + schar
			else:
				firstname = uts.random_name(random.randint(2, 4)) + schar + uts.random_name(random.randint(2, 4))
		...
		for person in persons:
			if person.role == 'staff':
				...
				utils.verify_ldap_object(
					person.dn,
					expected_attr={'uid': [person.username]},
