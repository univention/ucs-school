...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):  # formerly test_create_modify_delete_user_with_username_and_record_uid_and_UDM_property()
		...
		for role in ('student', 'teacher', 'staff', 'teacher_and_staff'):
			...
			person_a.update(record_uid=uts.random_name(), source_uid=source_uid, description=uts.random_name())
			person_b.update(record_uid=uts.random_name(), source_uid=source_uid, description=uts.random_name())
		...
		for person_a, person_b in person_list_pairs:
			self.log.info('Changing firstname of %r and lastname of %r.', person_a.username, person_b.username)
		...
		self.create_csv_file(person_list=person_list[1::2], mapping=config['csv']['mapping'], fn_csv=fn_csv)
		fn_config = self.create_config_json(values=config)
		self.save_ldap_status()
		self.run_import(['-c', fn_config, '-i', fn_csv], fail_on_preexisting_pyhook=False)
