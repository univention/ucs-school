...
def orderPrint(printer, userName, _file):
	...
	job = []
	cmds = [['lpr', '-P', printer, '-U', userName, _file], ["smbclient", "//%s/%s" % (master, printer), "-d3", "-U", "%s%%%s" % (userName, "univention"), "-c", "print %s" % _file]]
	...
	for cmd in cmds:
		...
		if not job:
			utils.fail(
				'Ordered print job was not stored in user spool directory')
	...
	with univention.testing.udm.UCSTestUDM() as udm:
		with utu.UCSTestSchool() as schoolenv:
			with ucr_test.UCSTestConfigRegistry() as ucr:
				...
				stu4, stu4_dn = schoolenv.create_user(
					school, username="%s2" %
					name)
