...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):
		...
		config.update_entry('scheme:record_uid', '<email>.<city>[0:4]'),  # lib attr that depends on UDM prop & lib attr
		config.update_entry('scheme:email', '<{}>@<maildomain><:strip>'.format(ext_attr_name)),  # lib attr that
		# depends on lib attr, not-lib-mapped extended attr and kwargs (maildomain)
		config.update_entry('scheme:username:default', '<firstname:lower>.<birthday>'),  # lib attr that depends on lib attrs
		...
		config.update_entry('scheme:{}'.format(ext_attr_name), "<firstname:lower>.<lastname:lower>")  # not-lib-mapped extended attr that depends on lib attrs
