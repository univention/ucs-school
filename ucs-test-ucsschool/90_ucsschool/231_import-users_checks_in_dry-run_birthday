...
class Test(CLI_Import_v2_Tester):
	def test(self):
		...
		self.log.info('*** 1.1 Importing (create) a new user of each role with birthday in correct format.')
		...
		self.log.info('*** 1.2 Importing (create) a new user of each role with birthday in correct format.')
		...
		for role in ('student', 'teacher', 'staff', 'teacher_and_staff'):
			person = Person(self.ou_A.name, role)
			person.update(record_uid=person.username, source_uid=source_uid, birthday=time.strftime('%Y-%m-%d'))
		...
		self.log.info('*** 3.1 Importing (modify) of existing users of each role with birthday in bad format.')
		...
		self.log.info('*** 3.2 Importing (modify) of existing users of each role with birthday in bad format.')
		...
		for person in person_list_success:
			person.update(birthday=person.old_birthday, school_classes={})  # As the import aborts, school classes may
		...
		self.log.info(
			...
			self.ou_A.name, self.ou_B.name, self.ou_C.name
		)
