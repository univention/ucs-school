...
class Test(CLI_Import_v2_Tester):
	...
	def test(self):
		...
		self.log.info('*** Importing same %r with username %r with dry-run (should fail)...', role, username)
		...
		self.log.info(
			'*** Importing (modify) same user (%r) but changing its username to %r in dry-run (should fail)...',
			person.username, role, username)
		config.update_entry('dry_run', True)
