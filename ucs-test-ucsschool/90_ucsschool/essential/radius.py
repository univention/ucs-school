...
def peap_auth(username, password, radius_secret, station_id=None):
...
def test_peap_auth(username, password, radius_secret, station_id=None, should_succeed=True):
	print '*** PEAP AUTH: user=%r, password=%r, should_succeed=%r, station_id=%r' % (username, password, should_succeed, station_id), '-' * 40
	auth_result = peap_auth(username, password, radius_secret, station_id)
	if auth_result != should_succeed:
		raise TestFail('PEAP authentication unexpected result (%r), while the expected is (%r)\nuser=%r, password=%r, should_succeed=%r, station_id=%r' % (
			auth_result, should_succeed, username, password, should_succeed, station_id))
