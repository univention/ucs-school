...
class Test(CLI_Import_v2_Tester):
	def _import_and_verify(self, config, person, delete=False):
		...
		if person.school == self.limbo_ou_name:
			if schools != [self.limbo_ou_name]:
				self.fail('Primary school is limbo_ou ({!r}) but "schools" attribute is {!r}.'.format(
					self.limbo_ou_name, schools))
			group_dns = self.lo.searchDn('(&(objectClass=univentionGroup)(uniqueMember={}))'.format(person.dn))
	...
	def test(self):
		...
		self.limbo_ou_name, self.limbo_ou_dn = self.schoolenv.create_ou(name_edudc=self.ucr.get('hostname'), use_cache=False)
		...
		for num, role in enumerate(('staff', 'student', 'teacher', 'teacher_and_staff'), start=1):
			...
			self.log.info('*** (%d/4) - 1. Importing (create) %r in school %r...', num, role, self.ou_A.name)
			...
			self.log.info('*** (%d/4) - 2. Importing (add OU) %r in school %r...', num, role, self.ou_B.name)
			...
			self.log.info('*** (%d/4) - 3. Importing (modify) %r in school %r...', num, role, self.ou_B.name)
			config.update_entry('school', self.ou_B.name)
			person.set_random_birthday()
			self._import_and_verify(config, person)
			self.log.info('OK: modification (A+B -> A+B) succeeded.')

			self.log.info('***  (%d/4) - 4. Importing (add OU) %r in school %r...', num, role, self.ou_C.name)
			config.update_entry('school', self.ou_C.name)
			person.update(school=self.ou_A.name, schools=[self.ou_A.name, self.ou_B.name, self.ou_C.name])
			self._import_and_verify(config, person)
			self.log.info('OK: create (A+B -> A+B+C) succeeded.')

			self.log.info('*** (%d/4) - 5. Importing (modify) %r in school %r...', num, role, self.ou_A.name)
			config.update_entry('school', self.ou_A.name)
			person.set_random_birthday()
			self._import_and_verify(config, person)
			self.log.info('OK: modification (A+B+C -> A+B+C) succeeded.')

			self.log.info('***  (%d/4) - 6. Deleting (del OU) %r in school %r...', num, role, self.ou_B.name)
			config.update_entry('school', self.ou_B.name)
			...
			self.log.info('***  (%d/4) - 7. Deleting (del OU) %r in school %r...', num, role, self.ou_A.name)
			config.update_entry('school', self.ou_A.name)
			person.update(school=self.ou_C.name, schools=[self.ou_C.name], school_classes={})
			self._import_and_verify(config, person, delete=True)
			self.log.info('OK: deletion (A+C -> C) succeeded.')

			self.log.info('***  (%d/4) - 8. Deleting (move) %r in (last) school %r...', num, role, self.ou_C.name)
			config.update_entry('school', self.ou_C.name)
			...
			self.log.info('*** (%d/4) - 9. Importing (move) %r in school %r...', num, role, self.ou_B.name)
			config.update_entry('school', self.ou_B.name)
			...
			self.log.info('***  (%d/4) - 10. Deleting (move) %r in (last) school %r...', num, role, self.ou_B.name)
			config.update_entry('school', self.ou_B.name)
