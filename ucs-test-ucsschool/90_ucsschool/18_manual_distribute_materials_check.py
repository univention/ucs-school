...
def main():
	with utu.UCSTestSchool() as schoolenv:
		with ucr_test.UCSTestConfigRegistry() as ucr:
			...
			# Create new project
			project = Distribution(school, sender=tea, connection=connection, ucr=ucr, files=[(filename, 'utf8')], recipients=[group], flavor='teacher')
			project.add()
			project.check_add()
			project.distribute()
			...
			filename = uts.random_string()
			# create new project
			project2 = Distribution(school, sender=tea2, connection=client2, ucr=ucr, files=[(filename, 'utf8')], recipients=[group], flavor='teacher')
			project2.add()
			...
			for distType in ['manual', 'automatic']:
				for collType in ['manual', 'automatic']:
					...
					new_group.create()
					project.put(description=new_description, distributeType=distType, distributeTime=distTime, distributeDate=distDate, collectType=collType, collectTime=collTime, collectDate=collDate, recipients=[new_group])
