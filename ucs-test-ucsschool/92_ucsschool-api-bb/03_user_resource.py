...
from univention.testing.ucsschool.bb_api import api_call, API_ROOT_URL, create_remote_static, HttpApiUserTestBase, RESSOURCE_URLS, partial_update_remote_static
...
class Test(HttpApiUserTestBase):
	...
	def test_07_move_teacher_one_school_only(self):
		self.logger.info('*** Going to move teacher from OU %r to %r ***', self.itb.ou_A.name, self.itb.ou_B.name)
		self.logger.info('*** Using OUs %r and %r.', self.itb.ou_A.name, self.itb.ou_B.name)
		...
		modify_attrs = {
			...
			'school_classes': {self.itb.ou_B.name: sorted([uts.random_username(4), uts.random_username(4)])},
		}
		self.logger.info('*** modify_attrs=%r', modify_attrs)

		resource_new = partial_update_remote_static((self.auth_headers, create_result['name'], modify_attrs))
		self.logger.info('*** API call (modify) returned: %r', resource_new)
	...
	def test_08_move_teacher_remove_primary(self):
		self.logger.info('*** Going to create teacher in OUs %r and %r, then remove it from primary (%r). ***', self.itb.ou_A.name, self.itb.ou_B.name, self.itb.ou_A.name)
		self.logger.info('*** Using OUs %r and %r.', self.itb.ou_A.name, self.itb.ou_B.name)
		create_attrs = self.make_user_attrs([self.itb.ou_A.name, self.itb.ou_B.name], partial=False, roles=('teacher',))
		self.logger.info('*** create_attrs=%r', create_attrs)
		self.assertEqual(create_attrs['school'].strip('/').split('/')[-1], self.itb.ou_A.name)
		self.assertEqual(
			[
		...
		self.assertSetEqual(set(create_attrs['school_classes'].keys()), {self.itb.ou_A.name, self.itb.ou_B.name})
		self.schoolenv.udm._cleanup.setdefault('groups/group', []).extend(self.extract_class_dns(create_attrs))
	...
	def test_09_move_teacher_remove_primary_with_classes(self):
		...
		self.logger.info('*** API call (modify) returned: %r', resource_new)
		self.assertEqual(create_result['name'], resource_new['name'])
		self.assertDictEqual(resource_new['school_classes'], {self.itb.ou_B.name: create_attrs['school_classes'][self.itb.ou_B.name]})
		self.logger.debug('*** zzz...')
		time.sleep(5)

		user = self.get_import_user(resource_new['dn'])
		self.logger.debug('*** user.school_classes=%r', user.school_classes)
		self.assertDictEqual(user.school_classes, {self.itb.ou_B.name: ["{}-{}".format(self.itb.ou_B.name, k) for k in create_attrs['school_classes'][self.itb.ou_B.name]]})
		self.assertEqual(create_result['name'], user.name)
		url = urljoin(RESSOURCE_URLS['users'], create_result['name'] + "/")
		self.assertEqual(resource_new["url"], url)
	...
	def test_11_move_teacher_remove_primary_with_classes_and_rename(self):
		self.logger.info('*** Going to create teacher in OUs %r and %r, then remove it from primary (%r) and rename it. ***', self.itb.ou_A.name, self.itb.ou_B.name, self.itb.ou_A.name)
		self.logger.info('*** Using OUs %r and %r.', self.itb.ou_A.name, self.itb.ou_B.name)
		create_attrs = self.make_user_attrs([self.itb.ou_A.name, self.itb.ou_B.name], partial=False, roles=('teacher',))
		self.logger.info('*** create_attrs=%r', create_attrs)
		self.assertEqual(create_attrs['school'].strip('/').split('/')[-1], self.itb.ou_A.name)
	...
	def test_12_modify_teacher_remove_all_classes(self):
		self.logger.info('*** Going to create teacher in OUs %r, then remove all its classes. ***', self.itb.ou_A.name)
	...
	def test_13_modify_classes_2old_2new(self):
		...
		self.logger.info('*** modify_attrs=%r', modify_attrs)
		self.schoolenv.udm._cleanup.setdefault('groups/group', []).extend(self.extract_class_dns(modify_attrs))
		resource_new = partial_update_remote_static((self.auth_headers, create_result['name'], modify_attrs))
		self.logger.info('*** API call (modify) returned: %r', resource_new)
		self.assertEqual(create_result['name'], resource_new['name'])
		...
		classes_without_ous = dict(
			(ou, [k.split("-", 1)[1] for k in kls])
			for ou, kls in user.school_classes.items()
		)
	...
	def test_14_modify_classes_0old_2new(self):
		...
		self.logger.info('*** Going to create %s in OUs %r. ***', role, self.itb.ou_A.name)
		self.logger.info('*** Using OUs %r.', self.itb.ou_A.name)
		create_attrs = self.make_user_attrs([self.itb.ou_A.name], partial=False, roles=(role,), school_classes={})
		self.logger.info('*** create_attrs=%r', create_attrs)
