...
from univention.testing.ucsschool.bb_api import API_ROOT_URL, HttpApiUserTestBase, RESSOURCE_URLS
...
class Test(TestCase):
	...
	def test_03_resources_in_api_root(self):
		...
		self.assertDictEqual(RESSOURCE_URLS, res, 'response.status_code = {} for URL {!r} -> {!r}'.format(
			response.status_code, response.url, response.text))
