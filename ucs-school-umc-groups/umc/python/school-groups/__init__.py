...
class Instance( Base ):
	...
	def colors( self, request ):
		...
		MODULE.info( 'school-groups.colors: options: %s' % str( request.options ) )
		...
		MODULE.info( 'school-groups.colors: result: %s' % str( allColors ) )
	...
	def query( self, request ):
		...
		"""
		MODULE.info( 'school-groups.query: options: %s' % str( request.options ) )
		...
		MODULE.info( 'school-groups.query: results: %s' % str( result ) )
	...
	def get( self, request ):
		...
		return: [ { 'id' : <unique identifier>, 'name' : <display name>, 'color' : <name of favorite color> }, ... ]
		"""
		MODULE.info( 'school-groups.get: options: %s' % str( request.options ) )
		ids = request.options
		result = []
		if isinstance( ids, ( list, tuple ) ):
			...
			MODULE.warn( 'school-groups.get: wrong parameter, expected list of strings, but got: %s' % str( ids ) )
			raise UMC_OptionTypeError( 'Expected list of strings, but got: %s' % str(ids) )
		MODULE.info( 'school-groups.get: results: %s' % str( result ) )
		self.finished( request.id, result )
