...
from ucsschool.lib.models.utils import env_or_ucr, exec_cmd
...
async def create_test_ou(ou_name, udm):
    return await create_ou(
        ...
        edu_name=f"edu{ou_name}",
        admin_name=f"adm{ou_name}",
        share_name=f"edu{ou_name}",
        lo=udm,
        baseDN=env_or_ucr("ldap/base"),
        hostname=env_or_ucr("hostname"),
        is_single_master=True,
        alter_dhcpd_base=False,
    )


@pytest.mark.asyncio
async def test_create_ou(udm_kwargs):
    """
    ...
    dns_after_ssh_school_creation = all_dns_1.difference(all_dns_0)
    ...
    dns_after_kelvin_school_creation = set(
        dn.replace(ou_name_kelvin, ou_name_ssh) for dn in dns_after_kelvin_school_creation
    )
...
async def test_attached_policies(udm_kwargs):
    ...
    async with UDM(**udm_kwargs) as udm:
        ...
        assert dhcp_dns_policy_dn in udm_obj.policies["policies/registry"]
