...
def create_objs_via_UDM_python_sequential(datas):  # type: (List[Dict[str, Any]]) -> List[str]
	res = []
	for data in datas:
...
def read_objs_via_UDM_python_parallel(dns, parallelism):  # type: (List[str], int) -> float
	# create `parallelism` amount of processes each working on a `num / parallelism` long list
...
def modify_objs_via_UDM_python_parallel(dns, parallelism):  # type: (List[str], int) -> float
	assert len(dns) % parallelism == 0
	if parallelism == 1:
		t0 = time.time()
		for dn in dns:
...
def delete_objs_via_UDM_python_parallel(dns, parallelism):  # type: (List[str], int) -> float
	assert len(dns) % parallelism == 0
	if parallelism == 1:
		t0 = time.time()
		for dn in dns:
