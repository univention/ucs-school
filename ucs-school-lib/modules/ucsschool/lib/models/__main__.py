...
def escape_filter_c(filter_s):  # type: (str) -> str
    filter_s = escape_filter_chars(filter_s)
    return filter_s.replace(r"\2a", "*")


def get_ldap_access():  # type: () -> Tuple[LoType, PoType]
    return getAdminConnection()


def load_model(model_name):  # type: (str) -> Type[UCSSchoolModel]
    try:
...
def get_obj(lo, model_cls, dn, name, school):
    # type: (LoType, Type[UCSSchoolModel], str, str, str) -> UCSSchoolModel
    try:
        if dn:
...
@click.pass_context
def cli(ctx, debug):  # type: (click.core.Context, bool) -> None
    ctx.obj["DEBUG"] = debug
...
def create(
    ...
    for k, v in single_value:
        if k not in model_cls._attributes:
            ...
            sys.exit(1)
        setattr(obj, k, v)
    to_append = {}  # type: Dict[str, Set[str]]
...
def modify(model, dn, name, school, multi_value, single_value, rm_value):
    ...
    to_remove = {}  # type: Dict[str, Set[str]]
    for k, v in rm_value:
        if k not in model_cls._attributes:
            logger.critical("Unknown attribute %r for model %r.", k, model_cls.__name__)
            sys.exit(1)
...
def delete(model, dn, name, school):  # type: (str, str, str, str) -> None
...
@click.pass_context
def list_objs(ctx, model, dn, name, school, attr_filters, filter_str, print_attrs):
    # type: (click.core.Context, str, str, str, str, Iterable[Tuple[str, str]], str, str) -> None
    logger.debug(
    ...
    def check_school(lo, school):  # type: (LoType, str) -> str
    ...
    def get_and_print_with_school(lo, model_cls, school, filter_str):
        # type: (LoType, Type[UCSSchoolModel], str, str) -> None
    ...
    def get_and_print_without_school(lo, model_cls, base="", filter_str=""):
        # type: (LoType, Type[UCSSchoolModel], str, str) -> None

        filter_str = udm_filter_from_school_filter(filter_str)
        model_cls.init_udm_module(lo)
        try:
            udm_objs = udm_modules.lookup(
    ...
    def get_and_print(lo, model_cls, school="", base="", filter_str=""):
        # type: (LoType, Type[UCSSchoolModel], str, str, str) -> None
        if school:
...
def list_models(attributes):  # type: (bool) -> None
