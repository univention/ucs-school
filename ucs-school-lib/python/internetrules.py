...
class Rule(object):
	...
	def addDomain(self, domain, idx=-1, listType=None):
		...
		if listType not in Rule._listTypesInv:
			listType = self.type
	...
	def save(self):
		...
		if not orgRule or orgRule.type != self.type:
			vars.append('%s/filtertype=%s' % (prefix, Rule._listTypesInv[self.type]))
		...
		for itype in Rule._listTypes.values():
			...
			domainPrefix = '%s/domain/%s' % (prefix, Rule._listTypesInv[itype])
			iorg = 0
			inew = 0
	...
	def load(name, userRule=False):
		...
		return Rule.list(name, userRule)
	...
	def list(filterName=None, userRule=False):
		...
		for k, v in ucr.items():
			...
			if imatch:
				...
				irule.userRule = bool(imatch.group('userPrefix'))
				if userRule != irule.userRule:
					continue
				...
				if iproperty == 'filtertype':
					if v not in Rule._filterTypes:
						irule.type = Rule.WHITELIST
						MODULE.error('Unknown filtertype "%s" for rule "%s", using whitelist as default.' % (v, irule))
					else:
						irule.type = Rule._filterTypes[v]
					...
					listType = Rule._listTypes.get(imatch.group('listType'))
