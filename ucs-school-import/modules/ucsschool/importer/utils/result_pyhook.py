...
class ResultPyHook(ImportPyHook):
	...
	* self.logger      # Python logging instance
	...
	def __init__(self, *args, **kwargs):
		"""
		If None is passed as LDAP connection object to ImportPyHooks,
		the constructor of ImportPyHooks will get an admin connection automatically.
		"""
		super(ImportPyHook, self).__init__(None, *args, **kwargs)  # None ==> lo

	def user_result(self, user_import):
		"""
		Run code after user import has finished. The importer object is passed
		to this hook, so result summaries etc are possible.

		:param Importer user_import: Importer that has finished its work
		:return: None
		"""
		return None
