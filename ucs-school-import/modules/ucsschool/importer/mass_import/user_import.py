...
class UserImport(object):
    ...
    async def create_and_modify_users(self, imported_users):
            ...
            try:
                ...
                try:
                    if user.action == "A":
                        ...
                        if self.dry_run:
                            await user.validate(
                                self.udm, validate_unlikely_changes=True, check_username=True
                            )
                            if self.errors:
                        ...
                        if self.dry_run:
                            await user.validate(
                                self.udm, validate_unlikely_changes=True, check_username=False
                            )
                            if self.errors:
                                raise ValidationError(user.errors.copy())
    ...
    async def find_importuser_in_ldap(self, import_user):  # type: (ImportUser) -> ImportUser
        ...
        """
        try:
            return await import_user.get_by_import_id(
                self.udm, import_user.source_uid, import_user.record_uid
            )
    ...
    async def determine_add_modify_action(self, imported_user):  # type: (ImportUser) -> ImportUser
        ...
        if (
            user.disabled != "0"
            or await user.has_expiry(self.udm)
            or await user.has_purge_timestamp(self.udm)
        ):
            self.logger.info(
    ...
    async def detect_users_to_delete(self):  # type: () -> List[Tuple[str, str, List[str]]]
        """
        ...
        ucs_user_ids = set(await self.get_ids_of_existing_users())
    ...
    async def delete_users(self, users=None):
        ...
        * To add or change a deletion strategy overwrite :py:meth:`do_delete()`.
        ...
        for num, (source_uid, record_uid, input_data) in enumerate(users, start=1):
            ...
            try:
                user = await a_user.get_by_import_id(self.udm, source_uid, record_uid)
    ...
    async def do_school_move(self, imported_user, user):  # type: (ImportUser, ImportUser) -> ImportUser
        ...
        """
        if self.dry_run:
            await user.check_schools(lo=self.udm, additional_schools=[imported_user.school])
        ...
        user = await imported_user.get_by_import_id(
            self.udm, imported_user.source_uid, imported_user.record_uid
        )
        return user
    ...
    async def do_delete(self, user):  # type: (ImportUser) -> bool
        ...
        if deletion_grace <= deactivation_grace:
            ...
            if deactivation_grace == 0:
                ...
                modified |= await self.set_deactivation_grace(user, deactivation_grace)
    ...
    async def set_deactivation_grace(self, user, grace):  # type: (ImportUser, int) -> bool
        """
        ...
        if user.disabled == "1":
            ...
            return False
        elif await user.has_expiry(self.udm):
            self.logger.info(
    ...
    async def set_deletion_grace(self, user, grace):  # type: (ImportUser, int) -> bool
        ...
        :rtype: bool
        """
        if await user.has_purge_timestamp(self.udm):
            self.logger.info(
