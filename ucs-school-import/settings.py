...
STATIC_URL = "/{}/".format(
    ucr.get("ucsschool/import/http_api/URL_path/static", "ucsschool-static").strip().strip("/")
)
MEDIA_ROOT = os.path.join(SPOOL_DIR, "media")  # uploads go here
MEDIA_URL = "/{}/".format(
    ucr.get("ucsschool/import/http_api/URL_path/media", "ucsschool-media").strip().strip("/")
)
