...
from univention.admin.uldap import access as LoType, getAdminConnection
...
def get_ip_iface(ip_address: str) -> IPv4Interface:
    try:
...
def mac_address_is_used(mac_address: str, lo: LoType) -> bool:
    result = lo.search(
        base=ucr["ldap/base"],
        scope="sub",
...
def import_computers(input_file: str) -> None:
    try:
    ...
    with open(input_file) as csv_file, stopped_notifier():
        ...
        for row in reader:
            ...
            if computer_type in computer_to_class_mapping:
                ...
                # zone is never saved -> todo create bug | discuss
                computer.zone = zone
