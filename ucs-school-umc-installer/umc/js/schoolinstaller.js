...
//			this._pages.school._footerButtons.next.set('label', _('Install'));
		...
		_initialQuery: function() {
			...
			this._initialDeferred = this.standbyDuring(tools.umcpCommand('schoolinstaller/query').then(lang.hitch(this, function(data) {
				...
				if (this._serverRole === 'domaincontroller_slave') {
					this._pages.setup.set('helpText', _('This wizard guides you step by step through the installation of a UCS@school Replica Directory Node. Before continuing please make sure that an UCS@school Primary Directory Node has already been set up for a multi server environment.'));
				}
